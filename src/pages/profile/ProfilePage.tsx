...
const ProfilePage = () => {
    ...
    const user = useSelector(
        (state: RootState) => state.authenticatedUser.user,
    );
    ...
    const [chosenMonth, setChosenMonth] = useState<string | undefined>(
        undefined,
    );
    ...
    const handleFilteredWorkouts = () => {
        ...
        const filteredByMonth = workouts.filter(
            (workout: Workout) =>
                format(workout.creationDate, "LLLL") === chosenMonth,
        );
    ...
    const handleDaySelection = (
        ...
        if (selectedDay === 0) {
            ...
            setFilteredByDay(
                workouts.filter(
                    (workout: Workout) =>
                        getDate(workout.creationDate) === selectedDay,
                ),
            );
        }
    ...
    const handleYearSelection = (
        ...
        if (selectedYear === 0) {
            ...
            setFilteredByYear(
                workouts.filter(
                    (workout: Workout) =>
                        getYear(workout.creationDate) === selectedYear,
                ),
            );
        }
        handleFilteredWorkouts();
